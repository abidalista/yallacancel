...
export default function PaywallModal({
  ...
  function submitAccessCode() {
    ...
    if (/^pay_[A-Za-z0-9]+$/.test(raw)) {
      setCodeError(false);
      track("checkout_started", { locale, plan_id: planId, method: "pay_receipt" });
    ...
    setCodeError(false);
    track("checkout_started", { locale, plan_id: planId, method: "access_code" });
    onPaymentSuccess(`founder_${code}`);
  }

  function startCheckout() {
    track("checkout_started", { locale, plan_id: planId, method: "whop" });
    setShowCheckout(true);
  }

  function handleClose() {
    track("paywall_dismissed", { locale, plan_id: planId, saw_checkout: showCheckout });
    onClose();
  }
